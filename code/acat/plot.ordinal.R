...
results <- fread("02_analysis/ACAT/acat.results.ordinal.txt")
...
suggestive <- 5e-4
p1 <- results[, ggplot(.SD, aes(cum.bp, -log10(ACAT.P), color=factor(Chromosome %% 2)))] +
  ...
  scale_x_continuous(label=cum.bp$Chromosome[seq(1, 22, 2)], breaks = cum.bp$center[seq(1, 22, 2)]) + 
...
ggsave("02_analysis/ACAT/ordinal.plot.png", p1, units = "in", width = 12, height = 6)
