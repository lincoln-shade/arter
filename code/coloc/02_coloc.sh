...
pheno=$2
regression=$3
cat $chrs | xargs -n 1 -P 10 -I % Rscript code/coloc/02a_gwas.sumstats.R $pheno "data/tmp/chr%_maf.frq" $regression %

# merge gwas and qtl data
cat data/tmp/top_qtls.tmp | awk 'NR>1 {print$1,$2,$3,$4}' | xargs -P 10 -L 1 Rscript code/coloc/02b_basc.qtl.R

# run coloc
out_folder=$4
touch "$out_folder"coloc_results.tmp
p12=0.00001
