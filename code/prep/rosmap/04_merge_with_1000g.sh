...
plink \
  --bfile data/tmp/rosmap_unrelated.tmp \
  --biallelic-only strict \
  --make-bed \
  --out data/tmp/rosmap_unrelated_1.tmp

# write duplicate variant ids to file
cat data/tmp/rosmap_unrelated_1.tmp.bim | cut -f 2 | sort | uniq -d > data/tmp/rosmap_unrelated_1_dups.tmp.txt

# create plink fileset with only biallelic variants
plink \
  --bfile data/tmp/rosmap_unrelated_1.tmp \
  --biallelic-only strict \
  --exclude data/tmp/rosmap_unrelated_1_dups.tmp.txt \
  --make-bed \
  --out data/tmp/rosmap_unrelated_1a.tmp

# create list of SNPs from QCed rosmap
awk '{print$2}' data/tmp/rosmap_unrelated_1a.tmp.bim > data/tmp/rosmap_unrelated_1a_vars.tmp.txt

# create 1000g fileset with rosmap snps
plink \
  --bfile data/1000g/1000g \
  --extract data/tmp/rosmap_unrelated_1a_vars.tmp.txt \
  --make-bed \
  --biallelic-only strict \
...
plink \
  --bfile data/tmp/rosmap_unrelated_1a.tmp \
  --biallelic-only strict \
  --extract data/tmp/1000g1_vars.tmp.txt \
  --make-bed \
  --out data/tmp/rosmap_unrelated_1b.tmp
...
plink \
  ...
  --update-map data/tmp/rosmap_unrelated_1b.tmp.bim 4 2 \
  --make-bed \
...
plink \
  ...
  --a1-allele data/tmp/rosmap_unrelated_1b.tmp.bim 5 2 \
  --make-bed \
  --out data/tmp/1000g3.tmp

# 2 try to resolve strand issues
awk '{print$2,$5,$6}' data/tmp/rosmap_unrelated_1b.tmp.bim > data/tmp/rosmap_unrelated_1b_var.tmp.txt
awk '{print$2,$5,$6}' data/tmp/1000g3.tmp.bim > data/tmp/1000g3_var.tmp.txt
sort data/tmp/rosmap_unrelated_1b_var.tmp.txt data/tmp/1000g3_var.tmp.txt |uniq -u > data/tmp/rosmap_unrelated_1b_1000g3_var_diff.tmp.txt

awk '{print$1}' data/tmp/rosmap_unrelated_1b_1000g3_var_diff.tmp.txt | sort -u > data/tmp/rosmap_1000g_flip_list.tmp.txt
...
sort data/tmp/rosmap_unrelated_1b_var.tmp.txt data/tmp/1000g4_var.tmp.txt |uniq -u  > data/tmp/rosmap_1000g_uncorrected_var.tmp.txt
...
plink \
  --bfile data/tmp/rosmap_unrelated_1b.tmp \
  --exclude data/tmp/rosmap_1000g_exclude_var.tmp.txt \
  --make-bed \
  --out data/tmp/rosmap_unrelated_1c.tmp
