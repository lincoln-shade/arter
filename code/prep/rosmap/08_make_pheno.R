...
rosmap_pcs <- fread("data/rosmap/rosmap_unrelated_pca.eigenvec", header = F)
setnames(rosmap_pcs, 
...
##--------------------
# ordinal
rosmap_unrelated <- rosmap
save(rosmap_unrelated, file = "data/rosmap/rosmap_unrelated.RData")

# logistic 
rosmap[, arteriol_scler := ifelse(arteriol_scler < 2, 1, 2)]
write.table(rosmap, file = "data/rosmap/rosmap_unrelated_pheno.txt", row.names = F, col.names = T, quote = F)
write.table(rosmap[, -c("arteriol_scler")], file = "data/rosmap/rosmap_unrelated_covar.txt", row.names = F, col.names = T, quote = F)
