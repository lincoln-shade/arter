...
cat data/tmp/nacc_adgc_qc3.tmp.bim | awk 'gsub(/:[ACTG]:[ACTG]/, "", $2)' > data/tmp/nacc_adgc_qc3_varID_noA1A2.tmp.bim

# create plink fileset with only biallelic variants
plink \
  --bfile data/tmp/nacc_adgc_qc3.tmp \
  --bim data/tmp/nacc_adgc_qc3_varID_noA1A2.tmp.bim \
  --biallelic-only strict \
  --make-bed \
  --out data/tmp/nacc_adgc_qc3a.tmp

# write duplicate variant ids to file
cat data/tmp/nacc_adgc_qc3a.tmp.bim | cut -f 2 | sort | uniq -d > data/tmp/nacc_adgc_qc3a_dups.tmp.txt

# create plink fileset with only biallelic variants
plink \
  --bfile data/tmp/nacc_adgc_qc3a.tmp \
  --bim data/tmp/nacc_adgc_qc3_varID_noA1A2.tmp.bim \
  --biallelic-only strict \
  --exclude data/tmp/nacc_adgc_qc3a_dups.tmp.txt \
  --make-bed \
  --out data/tmp/nacc_adgc_qc3b.tmp

# create list of SNPs from QCed NACC_ADGC
awk '{print$2}' data/tmp/nacc_adgc_qc3b.tmp.bim > data/tmp/nacc_adgc_qc3b_vars.tmp.txt

# create 1000g fileset with NACC_ADGC snps
plink \
  --bfile data/1000g/1000g \
  --extract data/tmp/nacc_adgc_qc3b_vars.tmp.txt \
  --make-bed \
  --biallelic-only strict \
...
plink \
  --bfile data/tmp/nacc_adgc_qc3b.tmp \
  --biallelic-only strict \
  --extract data/tmp/1000g1_vars.tmp.txt \
  --make-bed \
  --out data/tmp/nacc_adgc_qc3c.tmp
...
plink \
  ...
  --update-map data/tmp/nacc_adgc_qc3c.tmp.bim 4 2 \
  --make-bed \
...
plink \
  ...
  --a1-allele data/tmp/nacc_adgc_qc3c.tmp.bim 5 2 \
  --make-bed \
  --out data/tmp/1000g3.tmp

# 2 try to resolve strand issues
awk '{print$2,$5,$6}' data/tmp/nacc_adgc_qc3c.tmp.bim > data/tmp/nacc_adgc_qc3c_var.tmp.txt
awk '{print$2,$5,$6}' data/tmp/1000g3.tmp.bim > data/tmp/1000g3_var.tmp.txt
sort data/tmp/nacc_adgc_qc3c_var.tmp.txt data/tmp/1000g3_var.tmp.txt |uniq -u > data/tmp/nacc_adgc_qc3c_1000g3_var_diff.tmp.txt

awk '{print$1}' data/tmp/nacc_adgc_qc3c_1000g3_var_diff.tmp.txt | sort -u > data/tmp/nacc_adgc_1000g_flip_list.tmp.txt
...
sort data/tmp/nacc_adgc_qc3c_var.tmp.txt data/tmp/1000g4_var.tmp.txt |uniq -u  > data/tmp/nacc_adgc_1000g_uncorrected_var.tmp.txt
...
plink \
  --bfile data/tmp/nacc_adgc_qc3c.tmp \
  --exclude data/tmp/nacc_adgc_1000g_exclude_var.tmp.txt \
  --make-bed \
  --out data/tmp/nacc_adgc_qc3d.tmp
...
# merge
plink \
  --bfile data/tmp/nacc_adgc_qc3d.tmp \
