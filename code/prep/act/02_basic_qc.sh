...
plink \
  --bfile /data_global/ACT/qced/act_nhw_qced \
  --keep data/tmp/act_ids.txt \
  --make-bed \
...
plink \
  --bfile data/tmp/act.tmp \
  --geno 0.2 \
  --make-bed \
  --out data/tmp/act_qc1_geno.2.tmp

# 20% IID missingness threshold
plink \
  --bfile data/tmp/act_qc1_geno.2.tmp \
  --mind 0.2 \
  --make-bed \
  --out data/tmp/act_qc1_mind.2.tmp

# 5% variant missingness threshold
plink \
  --bfile data/tmp/act_qc1_mind.2.tmp \
  --geno 0.05 \
  --make-bed \
  --out data/tmp/act_qc1_geno.05.tmp

# 5% IID missingness threshold
plink \
  --bfile data/tmp/act_qc1_geno.05.tmp \
  --mind 0.05 \
  --make-bed \
  --out data/tmp/act_qc1_mind.05.tmp

# filter variants by HWE test
# include non-controls because plink phenotypes aren't accurate
plink \
  --bfile data/tmp/act_qc1_mind.05.tmp \
  --hwe 1e-6 midp include-nonctrl \
  --make-bed \
  --out data/tmp/act_qc1_hwe.tmp

# create initial MAF 0.01 threshold
plink \
  --bfile data/tmp/act_qc1_hwe.tmp \
  --maf 0.01 \
  --make-bed \
  --out data/tmp/act_qc1.tmp

# create pruned SNP subset
# exclude regions of known high heterozygosity
plink \
  --bfile data/tmp/act_qc1.tmp \
  --exclude data/inversion.regions.txt \
  --indep-pairwise 15000 1500 0.2 \
  --out data/tmp/act_qc1_pruned.tmp
