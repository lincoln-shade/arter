...
age.min <- 80
np.adgc.80 <- np.adgc[NACCDAGE >= age.min]

# keep those with B-ASC grading
np.adgc.80[NACCARTE %in% c(8, 9), NACCARTE := NA]
np.adgc.80.basc <- np.adgc.80[!(is.na(NACCARTE))]
...
np.adgc.80.basc.noNP <- Exclusion.Criteria(np.adgc.80.basc)

# keep those that pass genotyping QC and only 1 relative in each related cluster
np.adgc.80.basc.noNP.pass.qc <- fread("../ADGC.HRC/80+/01_data/qc.files/nacc.pass.basic.qc.fam")

# keep those who are european
np.adgc.80.basc.noNP.pass.qc.eur <- fread("../ADGC.HRC/80+/01_data/data/pheno.txt")

n.np <- np[, .N]
n.adgc <- adgc[, .N]
n.np.adgc <- np.adgc[, .N]
n.np.adgc.80 <- np.adgc.80[, .N]
n.drop.np.adgc.80 <- np.adgc[NACCDAGE < age.min, .N]
n.np.adgc.80.basc <- np.adgc.80.basc[, .N]
n.drop.np.adgc.80.basc <- np.adgc.80[is.na(NACCARTE), .N]
n.np.adgc.80.basc.noNP <- np.adgc.80.basc.noNP[, .N]
n.drop.np.adgc.80.basc.noNP <- n.np.adgc.80.basc - n.np.adgc.80.basc.noNP
n.np.adgc.80.basc.noNP.pass.qc <- np.adgc.80.basc.noNP.pass.qc[, .N]
n.drop.np.adgc.80.basc.noNP.pass.qc <-n.np.adgc.80.basc.noNP - n.np.adgc.80.basc.noNP.pass.qc
n.np.adgc.80.basc.noNP.pass.qc.eur <- np.adgc.80.basc.noNP.pass.qc.eur[, .N]
n.drop.np.adgc.80.basc.noNP.pass.qc.eur <- n.np.adgc.80.basc.noNP.pass.qc - n.np.adgc.80.basc.noNP.pass.qc.eur

rm(list = ls()[!(grep("n.", ls()))])
