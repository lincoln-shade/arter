...
Rscript 06a_make.metal.R \
  ../ROSMAP/all/02_analysis/logistic/regression.assoc.logistic \
  ../ROSMAP/all/02_analysis/logistic/plink.clumped;
...
echo $snps | xargs -n 1 -P 4 bash -c '
  locuszoom \
    ...
    --pvalcol P \
    --delim space \
    --prefix tabs.figs/locuszoom/rosmap.all.logistic/$0
  ';
