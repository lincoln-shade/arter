## NACC and ROSMAP logistic regression meta-analysis
##-----------------------------------------------------
library(pacman)
...
load("../ADGC.HRC/80+/02_analysis/ordinal/nacc.results.RData")
results <- nacc.results %>% 
  StripAlleles(., "SNP")

top.snps <- fread("../ADGC.HRC/80+/02_analysis/ordinal/indep.top.snps.txt", header = F) %>% 
  StripAlleles(., "V1") %>% 
  .[, V1]

results[SNP %in% top.snps, is_annotate := "yes"]

ordinal.significant <- results[P.Ord < 0.05]   
...
manplot <- ggplot(snp_mod, aes(x=BPcum, y=-log10(P.Ord))) +
...
ggsave("tabs.figs/nacc.80+.ord.manhattan.png", manplot, units = "in", width = 12, height = 6)
