...
load("../ROSMAP/02_analysis/ordinal/rosmap.results.RData")
results <- rosmap.results %>% 
  StripAlleles(., "SNP")

top.snps <- fread("../ROSMAP/02_analysis/ordinal/indep.top.snps.txt", header = F) %>% 
  StripAlleles(., "V1") %>% 
  .[, V1]
...
ggsave("tabs.figs/rosmap.80+.ord.manhattan.png", manplot, units = "in", width = 12, height = 6)
